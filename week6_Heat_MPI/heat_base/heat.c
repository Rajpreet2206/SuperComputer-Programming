...
int main(int argc, char *argv[]) {
    ...
    int dims[2] = {0, 0};  // Let MPI decide the best distribution
    MPI_Dims_create(size, 2, dims);

    int periods[2] = {0, 0};  // No wraparound connections
    MPI_Comm cart_comm;
	...
	int global_res = param.act_res + 2;
	int local_res_x = global_res / dims[0];
	int local_res_y = global_res / dims[1];

	// Allocate memory for local subdomain
	double *local_u = (double *)malloc((local_res_x + 2) * (local_res_y + 2) * sizeof(double));
	double *local_uhelp = (double *)malloc((local_res_x + 2) * (local_res_y + 2) * sizeof(double));
